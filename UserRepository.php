    ...
    public function create(array $data) {
        ...
        return DB::transaction(function () use ($data) {
                    $user = parent::create([
                                ...
                                //'avatar_location' => 'avatar/user.png', //Added
                    ...
                    if ($user) {
                        ...
                        $user->assignRole($data['register_as']);
                        if($data['register_as'] == 'influencer'){
                            $crediti = new Crediti();
                            $crediti->user_id = $user->id;
                            $crediti->pacchetto = config('crediti.crediti');
                            $crediti->save();

                            $coins = new Coins();
                            $coins->user_id = $user->id;
                            $coins->pacchetto = config('coins.coins');
                            $coins->save();
                        }
                    }
...
//        }
    ...
    public function findOrCreateProvider($data, $provider) {
        ...
        Session::put('social', $data);

        /*if ($provider == 'facebook') {
            $social_avatar = file_get_contents($data->avatar . '&access_token=' . $data->token);
            $location = 'avatar/' . $user->uuid . '-' . time() . 'jpg';
            File::put('storage/' . $location, $social_avatar);
