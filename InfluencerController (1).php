    ...
    public function get_influencer($uuid){
       ...
       if($user){
            ...
            if($user->dettagli->tiktok){
                $canali['tiktok']['link'] = $user->dettagli->tiktok;
                $canali['tiktok']['count'] = $user->dettagli->tiktok_follower;
                $canali['tiktok']['icon'] = 'fab fa-tiktok';
                $canali['tiktok']['label'] = 'tiktok_follower';
            }
            if($user->dettagli->pinterest){
                $canali['pinterest']['link'] = $user->dettagli->pinterest;
                $canali['pinterest']['count'] = $user->dettagli->pinterest_follower;
                $canali['pinterest']['icon'] = 'fab fa-pinterest';
                $canali['pinterest']['label'] = 'pinterest_follower';
            }
            if($user->dettagli->snapchat){
                $canali['snapchat']['link'] = $user->dettagli->snapchat;
                $canali['snapchat']['count'] = $user->dettagli->snapchat_follower;
                $canali['snapchat']['icon'] = 'fab fa-snapchat';
                $canali['snapchat']['label'] = 'snapchat_follower';
            }
            if($user->dettagli->twitch){
                $canali['twitch']['link'] = $user->dettagli->twitch;
                $canali['twitch']['count'] = $user->dettagli->twitch_follower;
                $canali['twitch']['icon'] = 'fab fa-twitch';
                $canali['twitch']['label'] = 'twitch_follower';
            }
            if($user->dettagli->reddit){
                $canali['reddit']['link'] = $user->dettagli->reddit;
                $canali['reddit']['count'] = $user->dettagli->reddit_karma;
                $canali['reddit']['icon'] = 'fab fa-twitch';
                $canali['reddit']['label'] = 'reddit_karma';
            }
             if($user->dettagli->tumblr){
                $canali['tumblr']['link'] = $user->dettagli->tumblr;
                $canali['tumblr']['count'] = $user->dettagli->tumblr_follower;
                $canali['tumblr']['icon'] = 'fab fa-twitch';
                $canali['tumblr']['label'] = 'tumblr_follower';
            }
             if($user->dettagli->myspace){
                $canali['myspace']['link'] = $user->dettagli->myspace;
                $canali['myspace']['count'] = $user->dettagli->myspace_follower;
                $canali['myspace']['icon'] = 'fab fa-twitch';
                $canali['myspace']['label'] = 'myspace_follower';
            }
            ...
            $update_all = DB::table('recensioni')
