<div style="border-radius:5px;" class="kt-portlet kt-portlet--tabs kt-portlet--height-fluid">
    ...
    <div class="kt-portlet__body">
        ...
        <div class="kt-widget4">
            ...
            <div class="kt-widget4__item" style="padding: 20px;border: 1px solid #e72b38; border-radius:25px;">
                @if($item->users->avatar_location != '')
                <div>{{-- {{route('frontend.user.influencer.get',['uuid'=>$item->users->uuid])}} --}}
                    <a href="{{url('/social',['uuid'=>$item->users->username])}}"><img style="max-height:40px; max-width:40px; border-radius:20px;" src="{{asset('storage/'.$item->users->avatar_location)}}" alt=""></a>
                </div>
                @else
              
                <span class="kt-userpic kt-userpic--info kt-margin-r-5 kt-margin-t-5">
                     <a href="{{url('/social',['uuid'=>$item->users->username])}}"> <span>{{$item->users->first_name[0]}} {{$item->users->last_name[0]}}</span></a>
                </span>
                
                @endif
                <div style="margin-left:10px;" class="kt-widget4__info">
                    <a href="{{url('/social',['uuid'=>$item->users->username])}}" class="kt-widget4__username">
                ...
                <div class="modal fade" id="modal-{{$loop->index}}" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
                    <div class="modal-dialog" role="document">
                        <div class="modal-content">
                            ...
                            <div class="modal-body scrolldiv" style="height: 300px; overflow: auto">
                                {!! clean($item->offerta) !!}
                                @if($item->importo != '')
